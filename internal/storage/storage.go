...
type Storage interface {
	...
	GetLatestRefreshToken(ctx context.Context, userID uuid.UUID) (models.RefreshToken, error)
...
func (p *PostgresStorage) GetUserByID(ctx context.Context, userID uuid.UUID) (models.User, error) {
	...
	query := fmt.Sprintf("SELECT id, email, created_at FROM %s WHERE id=$1;", usersTable)

	err := p.db.QueryRow(ctx, query, userID).Scan(&user.ID, &user.Email, &user.CreatedAt)
	if err != nil {
...
func (p *PostgresStorage) ListRoles(ctx context.Context) ([]string, error) {
	const op = "storage.ListRoles"

	var roles []string
	query := fmt.Sprintf("SELECT name FROM %s;", rolesTable)

	rows, err := p.db.Query(ctx, query)
	if err != nil {
		return roles, fmt.Errorf("%s: %w", op, err)
	}
	defer rows.Close()
...
func (p *PostgresStorage) GetUserRoles(ctx context.Context, userID uuid.UUID) ([]string, error) {
	const op = "GetUserRoles"

	var roles []string
	query := fmt.Sprintf(`SELECT rl.name
  	FROM %s rl
  	JOIN %s ru ON rl.id = ru.role_id
 	WHERE ru.user_id = $1`, rolesTable, userRolesTable)
...
func (p *PostgresStorage) AssignRole(ctx context.Context, userID uuid.UUID, roleName string) error {
	...
	query := fmt.Sprintf(`INSERT INTO %s(user_id, role_id)
	SELECT $1, id FROM roles WHERE name = $2;`, userRolesTable)
	_, err := p.db.Exec(ctx, query, userID, roleName)
	if err != nil {
...
func (p *PostgresStorage) RemoveRole(ctx context.Context, userID uuid.UUID, roleName string) error {
	...
	query := fmt.Sprintf(`DELETE FROM %s
 	WHERE user_id = $1
   	AND role_id = (SELECT id FROM roles WHERE name = $2);`, userRolesTable)
...
func (p *PostgresStorage) GetLatestRefreshToken(ctx context.Context, userID uuid.UUID) (models.RefreshToken, error) {
...
func (p *PostgresStorage) RevokeAllRefreshTokensForUser(ctx context.Context, userID uuid.UUID) error {
